...
The userspace cyclical reads from the framebuffer and displays the pixels (for example copies the content to a vnc-server). To be efficient (not to do a syscall for every pixel with bpf_map_lookup_elem!!) the bpf-map is mmaped and accessed directly. Therefore a kernel >= 5.4 is needed!
